...
impl ReaderServer {
    ...
    pub fn listen(&self) -> Result<(), String> {
        ...
        let server = Server::http(&addr).map_err(|e| format!("Failed to start server on {}: {}", addr, e))?;
        println!("🚀 EBook-RS Reader Server listening on http://localhost:{}", self.port);
        println!("Press Ctrl+C to exit.");

        for request in server.incoming_requests() {
            let url_str = format!("http://localhost{}", request.url());
            let parsed_url = Url::parse(&url_str).unwrap_or_else(|_| Url::parse("http://localhost/").unwrap());
            ...
            match path {
                ...
                _ if path.starts_with("/api/book/section/") => {
                    ...
                    match book.get_section(idx) {
                        Ok(sec) => {
                            let header = Header::from_bytes(&b"Content-Type"[..], &b"text/html; charset=utf-8"[..]).unwrap();
                            let _ = request.respond(Response::from_string(&sec.processed_html).with_header(header));
                        }
                        Err(err) => {
                            let _ = request.respond(Response::from_string(err).with_status_code(StatusCode(404)));
                        }
                    }
                }
                "/api/book/search" => {
                ...
                "/api/book/cover" => {
                    ...
                    if let Some((bytes, mime)) = book.cover_image() {
                        ...
                        let _ = request.respond(Response::from_string("No cover found").with_status_code(StatusCode(404)));
                    }
                }
                "/api/annotations" => {
                    let book = self.book.lock().unwrap();
