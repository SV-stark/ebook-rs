...
fn test_real_pdf_sample_parsing() {
    ...
    if !Path::new(pdf_path).exists() {
        println!(
            "⚠️ Skipping test_real_pdf_sample_parsing: sample PDF file not found at {}",
            pdf_path
        );
        return;
    }
    ...
    #[cfg(feature = "pdf")]
    {
        ...
        println!(
            "   - Search 'Alice' in PDF: {} matches found",
            matches.len()
        );
        assert!(
            !matches.is_empty(),
            "Search for 'Alice' in Alice in Wonderland PDF should yield matches"
        );
    }
    ...
    #[cfg(not(feature = "pdf"))]
    {
        assert!(
            result.is_err(),
            "Opening PDF when 'pdf' feature is disabled should return an error"
        );
    }
}
